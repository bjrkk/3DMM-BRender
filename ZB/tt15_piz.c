...
static inline void __TRAPEZOID_PIZ2_RGB_555(struct scan_edge *edge, br_boolean is_forward, br_fixed_ls *z_val, br_int_32 *fb_index) {
			...
			while (fb_curr < endptr) {
				if (*z_ptr > BrFixedToInt(z_curr)) {
					*z_ptr = BrFixedToInt(z_curr);
					...
					br_uint_16 r = BrFixedToInt(temp_r) >> 3;
					br_uint_16 g = BrFixedToInt(temp_g) >> 3;
					br_uint_16 b = BrFixedToInt(temp_b) >> 3;

					((br_uint_16*)zb.colour_buffer)[fb_curr] = (r << 10) | (g << 5) | b;
				}
				z_curr += zb.pz.grad_x;
				z_ptr++;
				fb_curr++;
...
static inline void __TRAPEZOID_PIZ2I_RGB_555(struct scan_edge *edge, br_boolean is_forward, br_fixed_ls *z_val, br_int_32 *fb_index) {
	br_int_32 endptr = edge->i;
...
static inline void __SETUP_PI(struct temp_vertex_fixed *v0, struct temp_vertex_fixed *v1, struct temp_vertex_fixed *v2) {
	colour_row_width = zb.row_width / 2;
...
static inline void __PARAM_PI_DIRN(screen_scalar a, screen_scalar b, screen_scalar c, struct scan_parameter *param) {	
	...
	param->grad_x = SafeFixedMac2Div(
		b - a, zb.main.y,
		c - a, -zb.top.y,
		g_divisor
	);

	param->grad_y = SafeFixedMac2Div(
		c - a, zb.top.x,
		b - a, -zb.main.x,
		g_divisor
	);
...
void BR_ASM_CALL RawTriangle_PIZ2_RGB_555(struct temp_vertex_fixed *v0, struct temp_vertex_fixed *v1, struct temp_vertex_fixed *v2) {
	...
	__TRAPEZOID_PIZ2_RGB_555(&zb.top, is_forward, &z_val, &fb_index);
	__TRAPEZOID_PIZ2_RGB_555(&zb.bot, is_forward, &z_val, &fb_index);
}

void BR_ASM_CALL RawTriangle_PIZ2I_RGB_555(struct temp_vertex_fixed *v0, struct temp_vertex_fixed *v1, struct temp_vertex_fixed *v2) {
	struct temp_vertex_fixed *a, *b, *c;
	a = v0; 
	b = v1; 
	c = v2;
	...
	__TRAPEZOID_PIZ2I_RGB_555(&zb.top, is_forward, &z_val, &fb_index);
	__TRAPEZOID_PIZ2I_RGB_555(&zb.bot, is_forward, &z_val, &fb_index);
}

void BR_ASM_CALL TriangleRenderPIZ2_RGB_555(struct temp_vertex_fixed *v0, struct temp_vertex_fixed *v1, struct temp_vertex_fixed *v2) {
	RawTriangle_PIZ2_RGB_555(v0, v1, v2);
}

void BR_ASM_CALL TriangleRenderPIZ2I_RGB_555(struct temp_vertex_fixed *v0, struct temp_vertex_fixed *v1, struct temp_vertex_fixed *v2) {
	RawTriangle_PIZ2I_RGB_555(v0, v1, v2);
}