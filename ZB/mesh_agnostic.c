...
static inline void __VERTEX_TRANSFORM_PROJECT(br_vertex *vp, struct temp_vertex_fixed *tvp) {
	br_int_32 w = BrFixedMac3(
		zb.os_model_to_screen.m[0][3], vp->p.v[0],
		zb.os_model_to_screen.m[1][3], vp->p.v[1],
...
br_int_32 BR_ASM_CALL ZbOSFFVGroupCulled_A(br_face *fp, struct temp_face *tfp, int count) {
	...
	do {
		br_scalar dot = BrFixedFMac3(
			fp->n.v[0], fw.eye_m.v[0],
			fp->n.v[1], fw.eye_m.v[1],
			fp->n.v[2], fw.eye_m.v[2]
		);
