/* $Id: innerp.inc.c 1.4 1995/03/01 15:49:34 sam Exp $ */
/* Perspective texture mapped triangle renderer */

#define SHIFT 1

/* Macros for moving left, right, up and down in texture */
void BR_ASM_CALL FNAME(struct temp_vertex *_a, struct temp_vertex *_b,
											 struct temp_vertex *_c) {
	...
	if (PerspCheat) {
		...
		range = a->v[0] - b->v[0];
		if ((tmpi = a->v[0] - c->v[0]) > range)
			range = tmpi;
		if ((tmpi = b->v[0] - c->v[0]) > range)
			range = tmpi;
		if ((tmpi = b->v[0] - a->v[0]) > range)
			range = tmpi;
		if ((tmpi = c->v[0] - a->v[0]) > range)
			range = tmpi;
		if ((tmpi = c->v[0] - b->v[0]) > range)
			range = tmpi;

		if ((tmpi = c->v[1] - a->v[1]) > range)
			range = tmpi;
		range /= 0x10000;
	...
	u_base = a->comp[C_U];
	if (b->comp[C_U] < u_base)
		u_base = b->comp[C_U];
	if (c->comp[C_U] < u_base)
		u_base = c->comp[C_U];
	v_base = a->comp[C_V];
	if (b->comp[C_V] < v_base)
		v_base = b->comp[C_V];
	if (c->comp[C_V] < v_base)
		v_base = c->comp[C_V];
	u_base &= 0xffff0000;
	v_base &= 0xffff0000;
...
#define PARAM_SETUP(param, p)                                                  \
	{                                                                            \
		...
		dp2 = c->p - a->p;                                                         \
		param.grad_x =                                                             \
				SafeFixedMac2Div(dp1, zb.main.y, -dp2, zb.top.y, g_divisor);           \
		param.grad_y =                                                             \
				SafeFixedMac2Div(dp2, zb.top.x, -dp1, zb.main.x, g_divisor);           \
		param.current =                                                            \
				a->p + (g_divisor >= 0 ? param.grad_x / 2 : -param.grad_x / 2);        \
		param.d_nocarry =                                                          \
				BrFixedToInt(zb.main.grad) * param.grad_x + param.grad_y;              \
...
#define PARAM_SETUP(param, p)                                                  \
	{                                                                            \
		...
		param.grad_x =                                                             \
				BrFloatToFixed((dp1 * zb.main.y - dp2 * zb.top.y) / g_divisor);        \
		param.grad_y =                                                             \
				BrFloatToFixed((dp2 * zb.top.x - dp1 * zb.main.x) / g_divisor);        \
	...
	while (tsl.Eu >= (signed)tsl.iW) {
		tsl.Eu -= tsl.iW;
		tsl.source = incu(tsl.source, SIZE);
		tsl.udW += tsl.dWx;
		udW_nocarry += dW_nocarry;
	}
	while (tsl.Eu < 0) {
		tsl.Eu += tsl.iW;
		tsl.source = decu(tsl.source, SIZE);
		tsl.udW -= tsl.dWx;
		udW_nocarry -= dW_nocarry;
	}
	while (tsl.Ev >= (signed)tsl.iW) {
		tsl.Ev -= tsl.iW;
		tsl.source = incv(tsl.source, SIZE);
		tsl.vdW += tsl.dWx;
		vdW_nocarry += dW_nocarry;
	}
	while (tsl.Ev < 0) {
		tsl.Ev += tsl.iW;
		tsl.source = decv(tsl.source, SIZE);
		tsl.vdW -= tsl.dWx;
		vdW_nocarry -= dW_nocarry;
	}
#endif
