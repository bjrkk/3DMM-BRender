 ...
 *
 * $Id: loadnff.c 1.14 1995/03/20 21:32:06 sam Exp $
 * $Locker:  $
 *
...
static char rscid[] = "$Id: loadnff.c 1.14 1995/03/20 21:32:06 sam Exp $";
...
static int AddVertex(br_scalar x, br_scalar y, br_scalar z)
{
    ...
    for (vp = temp_vertex_hash[hash]; vp; vp = vp->hash_next)
        if (vp->v.v[X] == x && vp->v.v[Y] == y && vp->v.v[Z] == z)
            return vp->n;
...
static int AddFace(int v1, int v2, int v3, int patch)
{
    ...
    if (used_faces >= allocated_faces)
    {
        ...
        memcpy(fp, temp_faces, used_faces * sizeof(*fp));
...
br_model *BR_PUBLIC_ENTRY BrFmtNFFLoad(char *name)
{
    ...
    while (!BrFileEof(fh))
    {
        ...
        case 'v': {
            ...
            NextLine(fh);
            sscanf(line, "from %g %g %g", &x, &y, &z);
            NextLine(fh);
            sscanf(line, "at %g %g %g", &tx, &ty, &tz);
            NextLine(fh);
            sscanf(line, "up %g %g %g", &ux, &uy, &uz);
            NextLine(fh);
            sscanf(line, "angle %g", &angle);
            NextLine(fh);
            sscanf(line, "hither %g", &hither);
            NextLine(fh);
            sscanf(line, "resolution %d %d", &rx, &ry);
        }
        break;
        ...
        case 'l': {
            float x, y, z, r, g, b;
            n = sscanf(cp, "%g %g %g %g %g %g", &x, &y, &z, &r, &g, &b);
        }
        break;
        ...
        case 'f': {
            ...
            n = sscanf(cp, "%g %g %g %g %g %g %g %g", &r, &g, &b, &kd, &ks, &shine, &t, &ior);
        }
        break;
        ...
        case 'c': {
            ...
            NextLine(fh);
            n = sscanf(line, "%g %g %g %g", &bx, &by, &bz, &br);
            NextLine(fh);
            n = sscanf(line, "%g %g %g %g", &ax, &ay, &az, &ar);
        }
        break;
        ...
        case 'p': {
            ...
            for (i = 0; i < num_vertices; i++)
            {
                ...
                n = sscanf(line, "%g %g %g %g %g %g", &vx, &vy, &vz, &nx, &ny, &nz);
                ...
                if (i < 2)
                {
                    verts[i] = AddVertex(BrFloatToScalar(vx), BrFloatToScalar(vy), BrFloatToScalar(vz));
                }
                else
                {
                    verts[2] = AddVertex(BrFloatToScalar(vx), BrFloatToScalar(vy), BrFloatToScalar(vz));
    ...
    {
        model->vertices[vp->n].p.v[X] = vp->v.v[X];
        model->vertices[vp->n].p.v[Y] = vp->v.v[Y];
        model->vertices[vp->n].p.v[Z] = vp->v.v[Z];
    }
