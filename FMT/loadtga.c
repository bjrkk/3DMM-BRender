...
br_pixelmap *BR_PUBLIC_ENTRY BrFmtTGALoad(char *name, br_uint_32 flags)
{
    ...
    default:
        BR_ERROR2("%d bit TGA file '%s' not supported", TGAheader.bits, name);
        break;
    }
    ...
    if (TGAheader.colourmaptype)
    {
        ...
        default:
            BrFileAdvance(TGAheader.colourmaplength * ((TGAheader.colourmapbits) >> 3), fh);
            break;

        case (8):
            pm->map = BrPixelmapAllocate(BR_PMT_RGBX_888, 1, 256, NULL, 0);
            ...
            {
            case (16):
                for (i = TGAheader.colourmapstart; i < TGAheader.colourmaplength; i++)
                {
                    if (i >= 256)
                        break;
