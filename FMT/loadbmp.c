 ...
 *
 * $Id: loadbmp.c 1.4 1995/08/31 16:28:14 sam Exp $
 * $Locker:  $
 *
...
static char rscid[] = "$Id: loadbmp.c 1.4 1995/08/31 16:28:14 sam Exp $";
...
br_pixelmap *BR_PUBLIC_ENTRY BrFmtBMPLoad(char *name, br_uint_32 flags)
{
    ...
    case (sizeof(BITMAPINFOHEADER)):
        headersize = sizeof(BITMAPFILEHEADER) + bi.biSize;
        break;
    ...
    {
    case (1):
        BR_ERROR1("1 bit BMP '%s' not supported", name);
        break;
    ...
    case (8):
        ...
        {
        case (BI_RGB):
            compressed_line_func = (bi.biBitCount == 4) ? Uncompressed_4 : Uncompressed_8;
            break;
        case (BI_RLE4):
            compressed_line_func = Compressed_4;
            break;
        case (BI_RLE8):
            compressed_line_func = Compressed_8;
            break;
        case (BI_BITFIELDS):
            BR_ERROR1("BMP compression type BITFIELDS not supported in '%s'", name);
            break;
        }
        ...
        break;
    case (16):
        BR_ERROR1("16 bit BMP '%s' not supported", name);
        break;
    ...
    case (24):
        if (bi.biCompression == BI_RGB)
            ...
            BR_ERROR1("24 bit compressed file '%s' not supported", name);
    ...
    case (32):
        BR_ERROR1("32 bit BMP '%s' not supported", name);
        break;
    }
    if (bf.bfOffBits != 0)
    ...
    case (BI_RLE4):
    case (BI_RLE8):
        if (compressed_line_func(pm, 0, fh) == NULL)
...
int FileNumCol(void *tmp)
{
    ...
    case (8):
        ...
        break;
    default:
        return 0; /* 24 bit has no palette */
    }
}
...
static void *Uncompressed_24(br_pixelmap *pm, int line, void *fh)
{
    ...
    if ((pm->width * 3) % 4)
        BrFileAdvance(4 - ((pm->width * 3) % 4), fh);
    return pm;
}
...
static void *Compressed_8(br_pixelmap *pm, int line, void *fh)
{
    ...
    for (dx = 0, dy = pm->height - 1;;)
    {
        ...
        if (RLEcontrol.control == RLE_ESCAPE)
        {
            ...
            case (RLE_EOL):
                dx = 0;
                memcpy((char *)pm->pixels + dy-- * pm->row_bytes, mblock, pm->width);
