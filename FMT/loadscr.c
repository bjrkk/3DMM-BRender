...
static void LexAdvanceReal(void)
{
    ...
    for (;;)
    {
        ...
        else if (CurrentFile.next_char == '"')
        {
            ...
            {
                CurrentFile.next_char = BrFileGetChar(CurrentFile.handle);
                if (CurrentFile.next_char == '"' || CurrentFile.next_char == '\n' || CurrentFile.next_char == BR_EOF)
                    break;
        ...
        else if (isalpha(CurrentFile.next_char) || CurrentFile.next_char == '_')
        {
            ...
            {
                CurrentFile.next_char = BrFileGetChar(CurrentFile.handle);
                if (!(isalpha(CurrentFile.next_char) || isdigit(CurrentFile.next_char) || CurrentFile.next_char == '_'))
                    break;
                lex_buffer[n++] = CurrentFile.next_char;
            }
            lex_buffer[n] = '\0';
        ...
        else if (isdigit(CurrentFile.next_char) || CurrentFile.next_char == '-')
        {
            ...
            {
                CurrentFile.next_char = BrFileGetChar(CurrentFile.handle);
                if (!(isdigit(CurrentFile.next_char) || CurrentFile.next_char == '.'))
                    break;
                lex_buffer[n++] = CurrentFile.next_char;
            }
            lex_buffer[n] = '\0';
...
br_uint_32 ParseMatFlags(void)
{
    ...
    for (;;)
    {
        ...
        case T_ENVIRONMENT:
            f |= BR_MATF_ENVIRONMENT_I;
            break;
        case T_ENVIRONMENT_L:
        case T_ENVIRONMENT_LOCAL:
            f |= BR_MATF_ENVIRONMENT_L;
            break;
        case T_PERSPECTIVE:
            f |= BR_MATF_PERSPECTIVE;
            break;
        case T_DECAL:
            f |= BR_MATF_DECAL;
            break;
        case T_ALWAYS_VISIBLE:
            f |= BR_MATF_ALWAYS_VISIBLE;
            break;
        case T_TWO_SIDED:
            f |= BR_MATF_TWO_SIDED;
            break;
