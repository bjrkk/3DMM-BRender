...
br_pixelmap *BR_PUBLIC_ENTRY BrFmtIFFLoad(char *name, br_uint_32 flags)
{
    ...
    if (!memcmp(IFFheader.type, "FORM", 4) || !memcmp(IFFheader.type, "LIST", 4) || !memcmp(IFFheader.type, "CAT ", 4))
    {
        do
        {
            ...
            else if (!memcmp(block, IMAGE_DATA, 4))
            {
                ...
                for (i = 0; i < BMHD.h; i++)
                {
                    ...
                    if (!memcmp(IFFheader.subtype, "ILBM", 4) ||
                        (!memcmp(IFFheader.subtype, "PBM ", 4) && BMHD.nPlanes < 8))
                ...
                BrFileAdvance(block_length, fh); /* skip unknown block */
