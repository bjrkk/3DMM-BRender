 ...
 *
 * $Id: loadgif.c 1.5 1995/08/31 16:28:15 sam Exp $
 * $Locker:  $
 *
...
static char rscid[] = "$Id: loadgif.c 1.5 1995/08/31 16:28:15 sam Exp $";
...
br_pixelmap *BR_PUBLIC_ENTRY BrFmtGIFLoad(char *name, br_uint_32 flags)
{
    ...
    br_uint_32 global_palette_size, local_palette_size;
    int i;
...
static void CopyLine(br_pixelmap *pm, char *linebuffer, short line)
{
    if (line >= 0 && line < pm->height)
        memcpy((char *)pm->pixels + line * pm->row_bytes, linebuffer, pm->width);
}
