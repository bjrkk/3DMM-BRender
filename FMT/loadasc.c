 ...
 *
 * $Id: loadasc.c 1.10 1995/03/20 21:32:06 sam Exp $
 * $Locker:  $
 *
...
} Keywords[] = {K("Ambient light color:", K_AMBIENT_LIGHT) K("Named object: ", K_NAMED_OBJECT)
                    K("Tri-mesh,", K_TRI_MESH) K("Vertex list:", K_NOP) K("Vertex", K_VERTEX) K("Face list:", K_NOP)
                        K("Face", K_FACE) K("Smoothing:", K_SMOOTHING) K("Material:", K_MATERIAL)
                            K("Direct light", K_DIRECT_LIGHT) K("Position:", K_POSITION)
                                K("Light color:", K_LIGHT_COLOR) K("Camera ", K_CAMERA) K("Target ", K_TARGET)
...
br_uint_32 BR_PUBLIC_ENTRY BrFmtASCLoad(char *name, br_model **mtable, br_uint_16 max_models)
{
    ...
    while (!BrFileEof(fh))
    {
        ...
        case K_TRI_MESH:
             ...
             */
            sscanf(line_tail, " Vertices: %d Faces: %d", &nvertices, &nfaces);
        ...
        case K_FACE:
             ...
             */
            sscanf(line_tail, "%d: A:%d B:%d C:%d AB:%d BC:%d CA:%d", &fnum, &fa, &fb, &fc, &fab, &fbc, &fca);
            /*
             * Only process if things are consistent
             */
        ...
        case K_SMOOTHING:
            ...
            if (cface > 0)
            {
                ...
                while (i = strtol(line_tail, &line_tail, 0))
                {
